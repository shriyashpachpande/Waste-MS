...
import { FileText, Clock, CheckCircle, AlertCircle, ArrowRight, Calendar, User, MapPin } from "lucide-react";
...
export default function AllReportsList() {
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">All Waste Reports</h2>
        <p className="text-gray-500 dark:text-gray-400 mt-1">Overview of all reported issues across the system</p>
      </div>
      ...
      {reports.length > 0 ? (
            ...
            <motion.div variants={item} key={r._id}>
              ...
              >
                <div className="p-5 flex-1">
                  ...
                  <p className="text-gray-900 dark:text-white font-medium line-clamp-2 mb-3">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mb-3">
                    <div className="flex items-center gap-1">
                      <User size={12} />
                      <span className="truncate max-w-[100px]">{r.reporterId?.name || "Unknown"}</span>
                    </div>
                    {r.coords && (
                      <div className="flex items-center gap-1">
                        <MapPin size={12} />
                        <span>{r.coords.lat.toFixed(2)}, {r.coords.lon.toFixed(2)}</span>
                      </div>
                    )}
                  </div>

                  {r.photos && r.photos.length > 0 && (
                    <div className="h-32 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-900">
                      <img
                        src={r.photos[0]}
                        alt="Report evidence"
        ...
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">There are no waste reports in the system.</p>
        </div>
      )}
    </motion.div>
  );
